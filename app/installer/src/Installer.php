...
namespace SBWebApplication\Installer;

use Doctrine\DBAL\DBALException;
use Doctrine\DBAL\DriverManager;
use Doctrine\DBAL\Exception\ConnectionException;
use SBWebApplication\Application;
use SBWebApplication\Config\Config;
use SBWebApplication\Installer\Package\PackageManager;
...
class Installer
{
    ...
    /**
     * @var Application SBWebApplication Application instance
     */
    protected $app;
        ...
        try {
            ...
            try {
                ...
                if ($this->app->db()->getUtility()->tableExists('@system_config')) {
                    $status = 'tables-exist';
                    $message = __('Existing SBWebApplication installation detected. Choose different table prefix?');
                } else {
                    $status = 'no-tables';
                }
    ...
    public function install($config = [], $option = [], $user = [])
    {
        ...
        try {
            ...
            foreach (glob($this->app->get('path.packages') . '/*/*/composer.json') as $package) {
                ...
                if ($package->get('type') === 'sb_new_web-extension' || $package->get('type') === 'sb_new_web-theme' ) {
