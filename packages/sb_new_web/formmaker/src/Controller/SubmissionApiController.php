...
use SBWebApplication\Application\Exception;
use SBWebApplication\Util\ArrObject;
use SB\Formmaker\Model\Form;
use SB\Formmaker\Model\Field;
use SB\Formmaker\Model\Submission;
use SB\Formmaker\Submission\MailHelper;
use SB\Formmaker\Submission\CsvHelper;
...
//					$query->whereInSet('form_id', $form); //input cleaned?
		...
		$limit = (int)$limit ?: App::module('sb/formmaker')->config('submissions_per_page');
		$count = $query->count();
	...
	public function saveAction ($data, $id = 0, $gRecaptchaResponse ='') {
		...
		if ($form->get('recaptcha')
			and $id == 0
			and ($recaptha_secret_key = App::module('sb/formmaker')->config('recaptha_secret_key'))) {
