 ...
 * @var SBVideo\Video\Model\Project|null $previous
 * @var SBVideo\Video\Model\Project|null $next
 */

$view->script('video', 'sbvideo/video:app/bundle/video.js', ['uikit-lightbox']);
...
?>
<article id="video-project">

	<?php if (in_array($config['project']['show_navigation'], ['both', 'top']) && ($next || $previous)) : ?>
		<ul class="uk-pagination">
			<?php if ($previous) : ?>
				<li class="uk-pagination-previous"><a href="<?= $app->url('@video/id', ['id' => $previous->id]) ?>">
						<i class="uk-icon-arrow-left uk-margin-small-right"></i><?= $previous->title ?></a></li>
			<?php endif; ?>
			<?php if ($next) : ?>
				<li class="uk-pagination-next"><a href="<?= $app->url('@video/id', ['id' => $next->id]) ?>">
		...
		<?php foreach ($project->images as $image) : ?>
			<div>
					<figure class="<?= $config['project']['overlay'] ?>">
						<img src="<?= $view->videoimage('url', [$image, $config['project']['thumbsize']]) ?>" alt="<?= $image['title'] ?>"
