...
namespace SBVideo\Video\Console\Commands;
...
class TranslateCommand extends Command
{
     ...
     * {@inheritdoc}
     */
    protected $name = 'video:translate';

    /**
     * {@inheritdoc}
     */
    protected $extension = 'video';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		$filename = $rootpath . '/packages/sbvideo/'.$this->extension.'/languages/'.$locale.'/messages.php';
		...
		$existing = [];
		//other packages
		$paths = glob($rootpath . '/packages/sbvideo/*/languages/'.$locale.'/messages.php', GLOB_NOSORT) ?: [];
		foreach ($paths as $p) {
			if ($p != $filename) {
		...
		if (file_exists($rootpath . '/packages/sbvideo/' . $this->extension . '/languages/base.php')) {
			$extraStrings = include($rootpath . '/packages/sbvideo/' . $this->extension . '/languages/base.php');
			foreach ($extraStrings as $string) {
				$translations[$string] = $locale != 'en_US' && isset($existing[$string])? $existing[$string] : $string;
			}
		}
		//get pot strings
		$potString = file_get_contents($rootpath . '/packages/sbvideo/'.$this->extension.'/languages/messages.pot');
	...
	protected function getFieldStrings (&$translations) {
		...
		$settingsPath = $this->container->path() . '/packages/sbvideo/' . $this->extension . '/app/settings';
		$fieldtypesPath = $this->container->path() . '/packages/sbvideo/' . $this->extension . '/fieldtypes';
