...
class ProfileUser implements \JsonSerializable
{
	...
	public function getProfile () {
		if (!isset($this->data)) {
			$this->data = [];
			$this->fieldValues = App::module('sb/userprofile')->getProfile($this->user, false, false);
    ...
    /**
     * @param \SBWebApplication\Site\Model\Node $node
     * @return string
     */
    ...
    public function getAvatar ($width = 280, $height = 280) {
        ...
        $fallback = $config['fallback_image_src'] ?: 'packages/sb_new_web/pk-framework/assets/noimage.jpg';
        return sprintf('<img height="%d" width="%d" alt="%s" src="%s">',
