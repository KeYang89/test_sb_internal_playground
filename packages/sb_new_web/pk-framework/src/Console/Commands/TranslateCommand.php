...
/**
 * Create .pot file:
 *  `php ./sb_new_web extension:translate sb/pk-framework`
 * Create .php files
 *  `php ./sb_new_web framework:translate [en_US]`
 */
namespace SB\PkFramework\Console\Commands;
...
class TranslateCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		$filename = $rootpath . '/packages/sb_new_web/'.$this->extension.'/languages/'.$locale.'/messages.php';
		...
		$existing = [];
		//other packages
		$paths = glob($rootpath . '/packages/sb_new_web/*/languages/'.$locale.'/messages.php', GLOB_NOSORT) ?: [];
		foreach ($paths as $p) {
			if ($p != $filename) {
		...
		foreach (['base', 'fields'] as $extra) {
			if (file_exists($rootpath . '/packages/sb_new_web/' . $this->extension . '/languages/' . $extra . '.php')) {
				$extraStrings = include($rootpath . '/packages/sb_new_web/' . $this->extension . '/languages/' . $extra . '.php');
		...
		//get pot strings
		$potString = file_get_contents($rootpath . '/packages/sb_new_web/'.$this->extension.'/languages/messages.pot');
	...
	protected function getFieldStrings (&$translations) {
		...
		$settingsPath = $this->container->path() . '/packages/sb_new_web/' . $this->extension . '/app/settings';
		$fieldtypesPath = $this->container->path() . '/packages/sb_new_web/' . $this->extension . '/fieldtypes';
